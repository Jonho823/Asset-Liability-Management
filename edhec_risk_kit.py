...
# Wrangle EDHEC Hedge Fund Index Returns dataset
def get_hfi_returns():
    """
...
# Compute semideviation (ref to lab session)
def semideviation(r):
    """
    Lab session version
...
# Compute semideviation (ref to Prof Martellini)
...
# Compute skewness in returns distribution
def skewness(r):
    """
...
# Compute kurtosis in returns distribution
def kurtosis(r):
    """
...
def is_normal(r, level=0.01):
    ...
    Test is applied at the 1% level (critical value) by default
...
# Compute Parametric VaR - Gaussian & Corner-Fisher VaR
...
# Compute weighted portfolio volatility
...
# Line plot for 2-asset efficient frontier
...
# Find set of weights for a set of target returns
...
def gmv(cov):
    ...
    Imagine all E(r) is the same (i.e. 1) -> can only reduce vol to improve Sharpe ratio
    """
    n = cov.shape[0]
...
# Generate Geometric Brownian Motion simulation
...
# Compute PV of a sequence of liabilities
def pv(flows, r):
    """
...
# Generate random interest rate and price with CIR model
import math
...
# Duration matching of target with a long bond & a short bond
...
# Back test on mixing of 2 set of returns
